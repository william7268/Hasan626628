...
from database import (
    ...
    get_last_sold_emails_to_user, # تأكد إن هادي الدالة مستوردة
    connect_db
)
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    initialize_db() # تهيئة قاعدة البيانات إذا لم تكن مهيأة (للتأكد فقط)

    if user_id == DEVELOPER_CHAT_ID:
        add_user(user_id, role='admin')
        user_data = get_user(user_id) # جلب البيانات المحدثة
    else:
    ...
    if user_data:
        ...
        # حالة نادرة لو فشل إضافة المستخدم
...
async def add_american_emails_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    يبدأ عملية إضافة الإيميلات الأمريكية ويطلب من المشرف إدخالها.
    """
    user_id = update.effective_user.id
    if user_id != DEVELOPER_CHAT_ID: # التحقق إذا كان المستخدم مشرفاً
        await update.message.reply_text("عذراً، هذا الأمر مخصص للمشرفين فقط.")
        return ConversationHandler.END # ينهي المحادثة إذا لم يكن مشرفاً
    ...
    return ADD_AMERICAN_EMAILS_STATE # ينتقل للحالة التي تنتظر الإيميلات

async def receive_american_emails(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    يستقبل الإيميلات من المشرف ويخزنها في قاعدة البيانات.
    """
